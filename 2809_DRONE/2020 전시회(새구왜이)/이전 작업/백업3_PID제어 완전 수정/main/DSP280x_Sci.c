...
interrupt void SCIB_ISR()
{
	...
	sci_buf = SCIx_RxChar_B();

	if(sci_buf == '\n')
	{
		sscanf(IMU_UC_buf, "*%lf,%lf,%lf\n", &IMU_f32_roll, &IMU_f32_pitch, &IMU_f32_yaw);

		IMU_iq17_roll = _IQ17(IMU_f32_roll);
		IMU_iq17_pitch = _IQ17(IMU_f32_pitch);
		IMU_iq17_yaw = _IQ17(IMU_f32_yaw);
		
		memset((void *)IMU_UC_buf,0x00,sizeof(unsigned char)*25);
	}
	else
		strncat((char*)IMU_UC_buf, (char*)&sci_buf, 1);
