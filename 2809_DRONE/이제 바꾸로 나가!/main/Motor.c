...
{	
	...
	//-180 ~ 180 (FL)우향 (FR)좌향 (FL)좌향 (RR)우향 좌회전 Error_value : + 우회전 Error_value : - 
	...
	Err_f32_FL[4]	= Err_f32_FL[3];
	Err_f32_FL[3]	= Err_f32_FL[2];
	Err_f32_FL[2]	= Err_f32_FL[1];
	Err_f32_FL[1]	= Error_f32_FrontLeft;
	Err_f32_FL[0]	= Err_f32_FL[1]		- Err_f32_FL[2];

	Err_f32_FR[4]	= Err_f32_FR[3];
	Err_f32_FR[3]	= Err_f32_FR[2];
	...
	//Kp_iq17_FL	= _IQmpy(_IQ(Kp), _IQ(Error_f32_FrontLeft));			Kp_iq17_FR	= _IQmpy(_IQ(Kp), _IQ(Error_f32_FrontRight));			
	...
	//Ki_iq17_FL	+= _IQmpy(_IQmpy(_IQ(Ki),  _IQ(Error_f32_FrontLeft)), _IQ(0.01));		Ki_iq17_FR	+= _IQmpy(_IQmpy(_IQ(Ki),  _IQ(Error_f32_FrontRight)), _IQ(0.01));		
	//Ki_iq17_YAW	+= _IQmpy(_IQmpy(_IQ(Ki_count),  Error_iq17_yaw), _IQ(0.01));
	
		//D제어 
	Kd_iq17_FL	= _IQdiv(_IQmpy(_IQ(Kd_count), _IQ(Err_f32_FL[0])), _IQ(0.01));	Kd_iq17_FR	= _IQdiv(_IQmpy(_IQ(Kd_count), _IQ(Err_f32_FR[0])), _IQ(0.01));	
	Kd_iq17_YAW	= _IQdiv(_IQmpy(_IQ(Kd_YAW), Err_iq17_YAW[0]), _IQ(0.01));
	
	//Kd_iq17_FL	= _IQdiv(_IQmpy(_IQ(Kd), _IQ(Err_f32_FL[0])), _IQ(0.01));		Kd_iq17_FR	= _IQdiv(_IQmpy(_IQ(Kd), _IQ(Err_f32_FR[0])), _IQ(0.01));		
	//Kd_iq17_YAW	= _IQdiv(_IQmpy(_IQ(Kd_count), Err_iq17_YAW[0]), _IQ(0.01));
	
	//최종 모터 제어
	PID_U16_FL	= IQ_TO_UINT16(Kp_iq17_FL + Ki_iq17_FL + Kd_iq17_FL);
	PID_U16_FR	= IQ_TO_UINT16(Kp_iq17_FR + Ki_iq17_FR + Kd_iq17_FR);
	PID_U16_YAW	= IQ_TO_UINT16(Kp_iq17_YAW + Ki_iq17_YAW + Kd_iq17_YAW);
